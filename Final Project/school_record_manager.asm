...
prompt_grade    db "Enter student grade (0-100, whole numbers only): ",0
...
msg_no_decimal  db "Error: Decimal grades are not allowed! Please enter whole numbers only.",10,0

; NEW MESSAGES
msg_duplicate   db 10,"WARNING: Student '%s' already exists!",10,0
prompt_continue db "Do you want to add another student? [1=Yes, 0=No]: ",0
confirm_delete  db "Are you sure you want to delete? [1=Yes, 0=No]: ",0
msg_cancelled   db "Operation cancelled.",10,0
msg_goodbye     db 10,"Thank you for using Student Record Manager. Goodbye!",10,0
...
fmt_int         db "%d",0
fmt_check_decimal db "%d.%d",0  ; For checking if input contains decimal
...
tmp_grade       resd 1
tmp_decimal     resd 1  ; For decimal detection
choice          resd 1
found_count     resd 1
min_grade       resd 1
max_grade       resd 1
header_printed_flag resd 1
confirm_choice  resd 1
input_buffer    resb 64  ; Buffer for input validation
...
; NEW: Check if student name already exists
...
; NEW: Check if input contains decimal point
; Returns: eax = 1 if has decimal, 0 if not
check_for_decimal:
    push ebp
    mov ebp, esp
    push esi
...
; [1] Add Student (ENHANCED with duplicate check and decimal validation)
add_student:
    mov eax, [count_students]
...
add_start_input:
    ...
    cmp eax, 1
    jne add_prompt_grade_again

    ; Duplicate found - ask user
    push dword tmp_name
    push dword msg_duplicate
    call _printf
    add esp, 8

    push dword prompt_continue
    call _printf
    add esp, 4

    push dword confirm_choice
    push dword fmt_int
    call _scanf
    add esp, 8
    ...
    mov eax, [confirm_choice]
    cmp eax, 1
    je add_start_input    ; Loop back to enter new name
    jmp add_cancelled     ; User chose No
...
add_prompt_grade_again:
    ...
    ; Read as string first to check for decimals
    push dword input_buffer
    call _gets
    add esp, 4

    ; Check for decimal point in input
    call check_for_decimal
    cmp eax, 1
    je add_has_decimal
...
add_has_decimal:
    push dword msg_no_decimal
    call _printf
    add esp, 4
    jmp add_prompt_grade_again

add_cancelled:
    push dword msg_cancelled
    call _printf
    add esp, 4
    jmp menu_loop

; [2] Delete by Name (ENHANCED with confirmation)
delete_name:
    mov eax, [count_students]
    cmp eax, 0
    je no_records
...
; [3] Delete all failing students (ENHANCED with confirmation)
delete_failing:
    mov eax, [count_students]
    cmp eax, 0
    je no_records
